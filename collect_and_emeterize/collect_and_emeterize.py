...
LEGACY_TOTAL_COUNTER_PATH = _ROOT / 'total_counter.json'
...
SCALING_FACTORS = {
    'power': POWER_SCALE,            # Power (W) scale factor: store value * 10
    'energy': ENERGY_WH_TO_JOULE,    # Energy (Wh) to joules conversion: store value * 3600
}
...
def load_total_counter_state() -> Dict[str, Any]:
    ...
    if not TOTAL_COUNTER_PATH.exists() and LEGACY_TOTAL_COUNTER_PATH.exists():
        TOTAL_COUNTER_PATH.write_bytes(LEGACY_TOTAL_COUNTER_PATH.read_bytes())
        logging.info(
            f"Migrated total counter state from {LEGACY_TOTAL_COUNTER_PATH} to {TOTAL_COUNTER_PATH}"
        )
...
def _send_multicast(data: bytes, log_msg: str) -> None:
    ...
    send_udp_multicast(
        data, multicast_config['address'], multicast_config['port'], multicast_config['ttl']
    )
